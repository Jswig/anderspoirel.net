...
working on a data pipeline. Unit tests with well-formed synthetic rows won't do you much
good if the actual source data look like this:
...
|        16 | feat.       | internal |
```
In practice, this means I would end up writing a lot of queries like this one:          
```sql
...
limit 100
```
However, this always returns the sames rows — maybe the inconsistencies in `category`
are found  4,000 rather than 4 rows deep and will never show up in the result of this
query! It's tempting to assume that the most recent rows (or however else you chose to
restrict the results) are representive, but this assumption is often wrong when
...
On the other hand, spot checking more than a few hundred rows is impractical, especially
if the table is wide.

A better compromise is to randomly sample from your data[^1]:
```sql
...
Of course, this won't find everything, but I find random sampling to be very effective
at surfacing issues I wouldn't otherwise discover during development. In practice, this
looks like alternating between taking random sampling to discover new problematic rows,
and building a follow-up query to discover variations of this problem to help me iterate
on my pipeline's logic.
